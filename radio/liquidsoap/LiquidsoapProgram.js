...
class LiquidsoapProgram extends Program {
        ...
        if (this.PreShow) {
            ...
            if (
                ...
                AppContext.getInstance().Logger.debug(
                    'PreShow scheduler command is: ' + preShowSchedulerCmd
                );
            } else {
                ...
                this.LivePlaybackSchedulerMeta.PreShowAt = this.PreShowStartTime;

                // The second token (e.g. "job xxx at Thu Jun 29 20:24:58 2017")
                this.LivePlaybackSchedulerMeta.PreShowId = ret
                    .split('\n')[1]
                    .split(' ')[1];
            }
        }
    ...
    doUnscheduleProgram() {
        ...
        if (this.LivePlaybackSchedulerMeta && this.LivePlaybackSchedulerMeta.ShowId) {
            preShowUnschedulingCmd = 'atrm ' + this.LivePlaybackSchedulerMeta.PreShowId;
        }

        let showUnschedulingCmd = 'atrm ' + this.LivePlaybackSchedulerMeta.ShowId;
        ...
        if (
            ...
            if (preShowUnschedulingCmd) {
                AppContext.getInstance().Logger.debug(
                    'PreShow unscheduler command is: ' + preShowUnschedulingCmd
                );
            }
            AppContext.getInstance().Logger.debug(
                'Show unscheduler command is: ' + showUnschedulingCmd
            );
        } else {
            if (preShowUnschedulingCmd) {
                try {
